"""
PenetrationAgent — penetration specialist.

Active exploitation to validate findings (HITL required for destructive ops)

Ported from CyberStrikeAI agents/penetration.md (English, W10-S1).
Uses BaseAgent infrastructure for:
    ...
    - SSE event emission
    - System prompt loading from app/agents/penetration.md
...
Tool allowlist: ['sqlmap', 'metasploit', 'mimikatz']
...
class PenetrationAgent(BaseAgent):
    ...
    async def _decide_next(
        ...
        thought = "Active exploitation to validate findings (HITL required for destructive ops) on " + self.target + " (W10 stub). Task: " + task_summary
        ...
        observation = "Penetration (W10 stub): sqlmap confirmed SQLi on /api/users?id=1 (boolean-blind). Recommend --os-shell (requires HITL approval) for further exploitation."
