"""
PrivilegeEscalationAgent — privilege-escalation specialist.

Privilege escalation path enumeration (HITL required)

Ported from CyberStrikeAI agents/privilege-escalation.md (English, W10-S1).
Uses BaseAgent infrastructure for:
    ...
    - SSE event emission
    - System prompt loading from app/agents/privilege-escalation.md
...
Tool allowlist: ['linpeas', 'winpeas', 'mimikatz']
...
class PrivilegeEscalationAgent(BaseAgent):
    ...
    async def _decide_next(
        ...
        thought = "Privilege escalation path enumeration (HITL required) on " + self.target + " (W10 stub). Task: " + task_summary
        ...
        observation = "Privilege escalation (W10 stub): linpeas found SUID /usr/bin/find (GTFOBins: privesc). winpeas found unquoted service path 'C:\\Program Files\\VulnSvc\\service.exe'."
