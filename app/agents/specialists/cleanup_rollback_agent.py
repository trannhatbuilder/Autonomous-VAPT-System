"""
CleanupRollbackAgent — cleanup-rollback specialist.

Runs cleanup_scan.sh + verifies no persistence left behind

Ported from CyberStrikeAI agents/cleanup-rollback.md (English, W10-S1).
Uses BaseAgent infrastructure for:
    ...
    - SSE event emission
    - System prompt loading from app/agents/cleanup-rollback.md
...
class CleanupRollbackAgent(BaseAgent):
    """Runs cleanup_scan.sh + verifies no persistence left behind
    ...
    async def _decide_next(
        ...
        thought = "Runs cleanup_scan.sh + verifies no persistence left behind on " + self.target + " (W10 stub). Task: " + task_summary
        ...
        observation = "Cleanup (W10 stub): scripts/cleanup_scan.sh removed /tmp/sqlmap-*, ~/.msf6/loot/, ~/.msf6/logs/, nuclei output dirs. Verified no cron jobs, no systemd services, no SSH authorized_keys modifications. Audit log entry created."
