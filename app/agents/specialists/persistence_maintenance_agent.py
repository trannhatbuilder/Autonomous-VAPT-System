"""
PersistenceMaintenanceAgent — persistence-maintenance specialist.

Persistence mechanism evaluation (HITL required)

Ported from CyberStrikeAI agents/persistence-maintenance.md (English, W10-S1).
Uses BaseAgent infrastructure for:
    ...
    - SSE event emission
    - System prompt loading from app/agents/persistence-maintenance.md
...
Usage:
    ...
    agent = PersistenceMaintenanceAgent(
        scan_id="scan_abc123",
...
class PersistenceMaintenanceAgent(BaseAgent):
    """Persistence mechanism evaluation (HITL required)
    ...
    AGENT_NAME = "persistence-maintenance"
    ...
    async def _decide_next(
        ...
        thought = "Persistence mechanism evaluation (HITL required) on " + self.target + " (W10 stub). Task: " + task_summary
        ...
        observation = "Persistence (W10 stub): evaluated 3 mechanisms: 1) cron job, 2) systemd service, 3) SSH authorized_keys. All require HITL approval. Recommend documenting cleanup steps."
